...
class CircuitGrid(pygame.sprite.RenderPlain):
    ...
    def highlight_selected_node(self, wire_num, column_num):
        ...
        self.circuit_grid_cursor.rect.left = self.xpos + \
            GRID_WIDTH * (self.selected_column + 1)
        self.circuit_grid_cursor.rect.top = self.ypos + \
    ...
    def handle_input_x(self):
        ...
        if selected_node_gate_part == node_types.EMPTY:
            ...
            self.model.set_node(self.selected_wire,
                                self.selected_column, circuit_grid_node)
        self.update()
    ...
    def handle_input_rotate(self, radians):
        ...
        if selected_node_gate_part == node_types.X or \
            ...
            circuit_grid_node.radians = (
                circuit_grid_node.radians + radians) % (2 * np.pi)
            self.model.set_node(self.selected_wire,
                                self.selected_column, circuit_grid_node)
    ...
    def delete_controls_for_gate(self, gate_wire_num, column_num):
        control_a_wire_num = self.model.get_node(
            gate_wire_num, column_num).ctrl_a
        control_b_wire_num = self.model.get_node(
    ...
    def __init__(self, circuit_grid_model):
        ...
        for wire_num in range(circuit_grid_model.max_wires):
            pygame.draw.line(self.image, globals.BLACK,
                             ...
                             (self.rect.width - (GRID_WIDTH * 0.5),
                              (wire_num + 1) * GRID_HEIGHT),
                             LINE_WIDTH)
        ...
        if node_type == node_types.H:
            ...
            if node.ctrl_a >= 0 or node.ctrl_b >= 0:
                ...
                self.image, self.rect = resources.load_image(
                    'gates/rx_gate.png', -1)
                self.rect = self.image.get_rect()
                pygame.draw.arc(self.image, globals.MAGENTA,
                                self.rect, 0, node.radians % (2 * np.pi), 6)
                pygame.draw.arc(self.image, globals.MAGENTA, self.rect,
                                node.radians % (2 * np.pi), 2 * np.pi, 1)
            else:
                self.image, self.rect = resources.load_image(
                    'gates/x_gate.png', -1)
            ...
            self.image, self.rect = resources.load_image(
                'gates/trace_gate.png', -1)
        elif node_type == node_types.SWAP:
            self.image, self.rect = resources.load_image(
                'gates/swap_gate.png', -1)
        else:
    ...
    def __str__(self):
        ...
        for wire_num in range(self.max_wires):
            ...
            for column_num in range(self.max_columns):
                retval += str(self.get_node_gate_part(wire_num,
                              column_num)) + ', '
            ...
            if wire_idx != control_wire_num:
                ...
                if other_node:
                    if other_node.ctrl_a == control_wire_num or \
                        ...
                        print("Found gate: ",
                              self.get_node_gate_part(
                                  gate_wire_num, column_num),
                              " on wire: ", gate_wire_num)
        return gate_wire_num

# transforms user input gate diagram into an actual circuit
                    ...
                    if node.node_type == node_types.IDEN:
                        ...
                        if node.radians == 0:
                            if node.ctrl_a != -1:
                                if node.ctrl_b != -1:
                                    # Toffoli gate
                                    qc.ccx(qr[node.ctrl_a],
                                           qr[node.ctrl_b], qr[wire_num])
                                else:
                                    # Controlled X gate
                        ...
                        if node.ctrl_a != -1:
                            # Controlled Swap
                            qc.cswap(qr[node.ctrl_a],
                                     qr[wire_num], qr[node.swap])
                        else:
                            # Swap gate
...
class CircuitGridNode():
    ...
    def __str__(self):
        ...
        string += ', radians: ' + \
            str(self.radians) if self.radians != 0 else ''
