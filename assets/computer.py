    ...
    def update(self, ball):
        ...
        if 40 < ball.rect.x / globals.WIDTH_UNIT < 54:
            if current_time - self.last_brick_broken > globals.MEASUREMENT_COOLDOWN_TIME:
                self.brick_after_measurement()
                self.last_brick_broken = pygame.time.get_ticks()
        else:
            self.brick_before_measurement()
        ...
        if pygame.sprite.collide_mask(ball, self.bricks[self.measured_state]) and not self.bricks[self.measured_state].is_gone():
            ball.bounce()
